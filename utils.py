...
import json
import torch
from attn_processor import SkipAttnProcessor, MoEFluxAttnProcessor2_0
...
def init_transformer_adapter(transformer, 
                 cross_attn_cls=MoEFluxAttnProcessor2_0,
                 self_attn_cls=None,
                 cross_attn_dim=None,
                 num_attetnion_heads=24, 
                 attention_head_dim=128,
                 **kwargs):
    transformer_config = transformer.config
    attn_procs = {}
    save_path = "attn_processor_names.txt"

    for name in transformer.attn_processors.keys():
        print(name)
        pass

        if name.endswith("processor") and name.startswith("transformer_blocks"):
            attn_procs[name] = MoEFluxAttnProcessor2_0(num_attetnion_heads, attention_head_dim)
        
        if name.endswith("processor") and name.startswith("single_transformer_blocks"):
            attn_procs[name] = MoEFluxAttnProcessor2_0(num_attetnion_heads, attention_head_dim)
